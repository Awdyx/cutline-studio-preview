...
type IpcHandler = dyn Fn(Request<String>) + Send + 'static;
...
"#,
  ...
  if let Some(webview_configuration) = webview_attributes.webview_configuration {
    // wrymium: pass as void pointer — CEF ignores WKWebViewConfiguration
    let ptr = objc2::rc::Retained::into_raw(webview_configuration) as *mut std::ffi::c_void;
    webview_builder = webview_builder.with_webview_configuration(ptr);
  }
  ...
  if let Some(new_window_handler) = pending.new_window_handler {
    ...
    webview_builder = webview_builder.with_new_window_req_handler(move |url, features| {
      url
        .parse::<http::Uri>()
        .map(|_url| {
          // wrymium: CEF doesn't provide wry-native opener types (Retained<WKWebView> etc.)
          // so we skip the new_window_handler and just Allow the request.
...
fn inner_size(
  ...
  if !has_children && !webviews.is_empty() {
    ...
    // wrymium: webview() returns *mut c_void (NSView pointer), not Retained
    let view_ptr = webview.webview() as *const objc2_app_kit::NSView;
    if !view_ptr.is_null() {
      let view_frame = unsafe { (*view_ptr).frame() };
