...
//! On macOS, a CFRunLoopTimer drives `CefDoMessageLoopWork()` (Cutline: 120 Hz for ProMotion).
    ...
    impl App {
        fn on_before_command_line_processing(
            ...
            if let Some(cmd) = command_line {
                ...
                // Cutline Studio: allow rAF/compositor to match ProMotion (replaces WKWebView macos-fps plugin).
                #[cfg(target_os = "macos")]
                {
                    let no_fps_cap = CefString::from("disable-frame-rate-limit");
                    ImplCommandLine::append_switch(cmd, Some(&no_fps_cap));
                }
            }
        }
...
fn install_message_pump() {
    ...
    // Default wrymium uses 30 Hz; Cutline targets 120 Hz ProMotion / high-refresh externals.
    const INTERVAL: f64 = 1.0 / 120.0;
    ...
    wrymium_log!("[wrymium] macOS CFRunLoopTimer installed at 120fps (Cutline)");
}
